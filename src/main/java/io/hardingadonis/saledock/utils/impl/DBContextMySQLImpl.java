...
public class DBContextMySQLImpl implements IDBContext {
  ...
  public DBContextMySQLImpl() {
    try (InputStream input =
             DBContextMySQLImpl.class.getClassLoader().getResourceAsStream(
      ...
      if (input == null) {
        ...
        this.dbURL = "jdbc:mysql://" + prop.getProperty("config.jdbc.url") +
                     ":3306/" + prop.getProperty("config.jdbc.database_name");
  ...
  public Connection getConnection() {
    ...
    try {
      ...
      connection =
          DriverManager.getConnection(this.dbURL, this.user, this.password);
