  ...
  public Long count() {
    ...
    try {
      ...
      PreparedStatement smt =
          conn.prepareStatement("SELECT COUNT(*) FROM `product`");
  ...
  public String getTop10(Integer duration) {
    ...
    try {
      ...
      PreparedStatement smt = conn.prepareStatement(
          "SELECT `p`.`name`, SUM(`od`.`quantity`) AS `count` FROM `order_detail` `od` JOIN `order` `o` ON `o`.`id` = `od`.`order_id` JOIN `product` `p` ON `p`.`id` = `od`.`product_id` WHERE `o`.`created_at` BETWEEN DATE_SUB(CURDATE(), INTERVAL ? MONTH) AND CURDATE() GROUP BY `p`.`id` ORDER BY `count` DESC LIMIT 10;");
