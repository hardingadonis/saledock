  ...
  public Long count() {
    ...
    try {
      ...
      PreparedStatement smt =
          conn.prepareStatement("SELECT COUNT(*) FROM `customer`");
  ...
  public String getTop10(Integer duration) {
    ...
    try {
      ...
      PreparedStatement smt = conn.prepareStatement(
          "SELECT `c`.`name`, COUNT(`o`.`id`) AS `count` FROM `customer` `c` JOIN `order` `o` ON `c`.`id` = `o`.`customer_id` WHERE `o`.`created_at` BETWEEN DATE_SUB(CURDATE(), INTERVAL ? MONTH) AND CURDATE() GROUP BY `c`.`id` ORDER BY `count` DESC LIMIT 10;");
